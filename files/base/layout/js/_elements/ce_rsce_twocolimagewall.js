document.addEventListener("DOMContentLoaded", function (event) {
    ...
    function setImageWidth() {
        if (window.innerWidth >= 767) {
            var containers = document.querySelectorAll('.ce--imagetextwall:not(.container) .force-container');

            containers.forEach(function (container) {
                var imageCol = container.querySelector('.image--col');
                var imageColImage = container.querySelector('.image-col--inner');
                var kachelDivA = container.querySelector('.content-col--image + .kachel-column-width--indicator');
                var kachelDivB = container.querySelector('.image-col--inner + .kachel-column-width--indicator');
                ...
                if (imageColImage) {
                    imageColImage.style.width = (distance + (containerWidth * (imageWidth / 100))) - 16 + 'px';
                }

                if (contentColImage) {
                    contentColImage.style.width = (distance + (containerWidth * (contentWidth / 100))) - 16 + 'px';
                }

                if (kachelDivA) {
                    kachelDivA.style.width = (distance + (containerWidth * (imageWidth / 100))) - 16 + 'px';
                }

                if (kachelDivB) {
                    kachelDivA.style.width = (distance + (containerWidth * (contentWidth / 100))) - 16 + 'px';
                }
            });
        }
    }
