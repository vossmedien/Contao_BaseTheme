...
namespace Vsm\VsmStripeConnect\Service\User;
...
class UserCreationService
{
    ...
    public function createContaoUser(array $metadata): ?int
    {
        ...
        try {
            ...
            if (!empty($metadata['memberGroup']) && is_numeric($metadata['memberGroup'])) {
                ...
                $defaultGroup = $container ? $container->getParameter('vsm_stripe_connect.default_member_group') ?? null : null;
                if ($defaultGroup) {
    ...
    private function updateExistingMember(MemberModel $member, array $userData): int
    {
        ...
        $this->logger->info('updateExistingMember aufgerufen für: ' . $member->username, [
            'subscription_duration' => $userData['subscription_duration'] ?? 'nicht gesetzt',
            'duration' => $userData['duration'] ?? 'nicht gesetzt',
            'current_stop' => $member->stop ? date('Y-m-d H:i:s', $member->stop) : 'nicht gesetzt',
            'has_groups' => !empty($userData['groups'])
        ]);
        
        // Mitgliedschaftsdauer bestimmen - entweder aus subscription_duration ODER aus duration
        $duration = 0;
        
        // Versuche subscription_duration zu verwenden
        if (!empty($userData['subscription_duration']) && is_numeric($userData['subscription_duration'])) {
            ...
            $this->logger->info('Mitgliedschaftsdauer aus subscription_duration: ' . $duration);
        }
        
        // Falls subscription_duration nicht verfügbar oder 0 ist, versuche duration
        if ($duration <= 0 && !empty($userData['duration']) && is_numeric($userData['duration'])) {
            $duration = intval($userData['duration']);
            $this->logger->info('Mitgliedschaftsdauer aus duration: ' . $duration);
        }
        
        // Nur wenn eine gültige Dauer gefunden wurde, aktualisiere die Mitgliedschaft
        if ($duration > 0) {
            ...
            $this->logger->warning('Keine gültige Mitgliedschaftsdauer gefunden, keine Änderung am Ablaufdatum');
        }
    ...
    private function createNewMember(array $userData): ?int
    {
        try {
            ...
            // Mitgliedschaftsdauer aus subscription_duration ODER duration holen
            $duration = 0;
            
            // Versuche subscription_duration zu verwenden
            if (!empty($userData['subscription_duration']) && is_numeric($userData['subscription_duration'])) {
                $duration = intval($userData['subscription_duration']);
                $this->logger->info('Mitgliedschaftsdauer aus subscription_duration genommen: ' . $duration);
            }
            
            // Wenn keine subscription_duration gefunden wurde, versuche duration
            if ($duration <= 0 && !empty($userData['duration']) && is_numeric($userData['duration'])) {
                $duration = intval($userData['duration']);
                $this->logger->info('Mitgliedschaftsdauer aus duration genommen: ' . $duration);
            }
            
            // Nur wenn eine positive Dauer gefunden wurde, berechne ein Enddatum
            if ($duration > 0) {
                $endTime = strtotime('+' . $duration . ' months', $startTime);
            ...
            if (!empty($userData['groups'])) {
                ...
                if ($container && $container->hasParameter('vsm_stripe_connect.default_member_group')) {
                    $defaultGroup = $container->getParameter('vsm_stripe_connect.default_member_group');
                    if ($defaultGroup) {
